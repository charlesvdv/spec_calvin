...
class Storage {
  public:
    ...
    // of the object is replaced. Returns true if the write succeeds, or false
    // if it fails for any reason.
    virtual bool PutObject(const Key &key, Value *value, int64 txn_id = 0) = 0;

    // Removes the object specified by 'key' if there is one. Returns true if
    // the deletion succeeds (or if no object is found with the specified key),
    // or false if it fails for any reason.
