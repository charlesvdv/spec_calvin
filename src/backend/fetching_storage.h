...
class FetchingStorage : public Storage {
  ...
  private:
     ...
     * a separate disk layer. To make the callbacks play nice for now,
     * everything is being placed here. After November 1, we can swap out
     * backends.
     */
    ...
    aiocb *generateControlBlock(int fd, char *buf, const int size,
                                Operation op);
