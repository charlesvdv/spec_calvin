...
string ConfigReader::Value(std::string const &entry) {
    ...
    if (ci == ConfigReader::content_.end())
        throw "does not exist";

    return ci->second;
}
//
// string ConfigReader::Value(std::string const& section, std::string const&
// entry, double value) {
//  try {
...
//	  strs << dbl;
//	  std::string str = strs.str();
//    return content_.insert(make_pair(section+'/'+entry,
//    itoa(value))).first->second;
//  }
//}
//
// string ConfigReader::Value(std::string const& section, std::string const&
// entry, std::string const& value) {
//  try {
//    return Value(section, entry);
//  } catch(const char *) {
//    return content_.insert(make_pair(section+'/'+entry,
