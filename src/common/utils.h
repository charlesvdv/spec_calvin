...
static inline double StringToDouble(const string &s) { return atof(s.c_str()); }
...
public:
    ...
    inline void Push(const T &item) {
        Lock l(&back_mutex_);
        // Check if the buffer has filled up. Acquire all locks and resize if
        // so.
...
class MyAtomicMap {
  public:
    MyAtomicMap() { pthread_mutex_init(&mutex_, NULL); }
    ~MyAtomicMap() {}
