...
class ConnectionMultiplexer {
  public:
    // Create a ConnectionMultiplexer that establishes two-way communication
    // with Connections for every other node specified by '*config' to exist.
    explicit ConnectionMultiplexer(Configuration *config);

    // TODO(alex): The deconstructor currently closes all sockets. Connection
    //             objects, however, do not have a defined behavior for trying
    //             to send messages to the multiplexor after it has been
    //             destroyed.
    ~ConnectionMultiplexer();

    // Creates and registers a new connection with channel name 'channel',
    // unless the channel name is already in use, in which case NULL is
    // returned. The caller (not the multiplexer) owns of the newly created
    // Connection object.
  ...
  private:
    ...
    // Specifies channel requested to be deleted. Null if there is no
    // outstanding connection deletion request.
...
class Connection {
  public:
    ...
    // queued up to be delivered, GetMessageBlocking waits at most
    // 'max_wait_time' seconds for a message to arrive. If no message arrives,
    // false is returned. 'message->Clear()' is NOT called.
