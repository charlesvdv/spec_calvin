...
void Sequencer::RunReader() {
    ...
    for (uint i = 0; i < dc.size(); ++i) {
        LOG(0, " group size is " << dc.size() << ", init for node "
                                 << dc[i]->node_id);
    ...
    while (!deconstructor_invoked_) {
        ...
        if (batch_queue_.Pop(&batch_message)) {
            ...
            LOG(batch_message->batch_number(),
                " msg's dest node is " << batch_message->destination_node());
            got_batch = true;
        }
        if (got_batch == false) {
            Spin(0.001);
        ...
        for (int i = 0; i < batch_message->data_size(); i++) {
            ...
            for (it = txn.readers().begin(); it != txn.readers().end(); ++it) {
                // LOG(-1, " adding to_send of
                // "<<configuration_->PartLocalNode(*it)<<", origin is "<<*it);
        ...
        for (map<int, MessageProto>::iterator it = batches.begin();
            ...
            // std::cout<<"Putting "<<batch_number<<" into queue at
            // "<<GetUTime()<<std::endl;
            LOG(batch_number, " before sending batch message! Msg's dest is "
                                  << it->second.destination_node() << ", "
                                  << it->second.destination_channel());
...
void Sequencer::output(DeterministicScheduler *scheduler) {
    ...
    myfile << "LATENCY" << '\n';
    myfile << latency_util.average_latency() << ", "
           << latency_util.medium_latency() << ", "
           << latency_util.the95_latency() << ", "
           << latency_util.the99_latency() << ", "
           << latency_util.the999_latency() << '\n';
    if (configuration_->this_node_id == 0) {
        ...
        myfile << avg_lat << ", " << med_lat << ", " << lat_95 << ", " << lat_99
               << ", " << lat_999 << '\n';
    } else if (configuration_->all_nodes[configuration_->this_node_id]
                   ->replica_id == 0) {
        // Pack up my data
