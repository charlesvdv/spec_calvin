...
void *GenepiSequencer::RunSequencerReader(void *arg) {
    reinterpret_cast<GenepiSequencer *>(arg)->RunReader();
    return NULL;
}

void *GenepiSequencer::RunSequencerWriter(void *arg) {
    reinterpret_cast<GenepiSequencer *>(arg)->RunWriter();
    return NULL;
}

GenepiSequencer::GenepiSequencer(Configuration *conf, ConnectionMultiplexer *multiplexer,
    ...
    : batch_count_(0), configuration_(conf), multiplexer_(multiplexer),
      ...
      queue_mode_(queue_mode), fetched_txn_num_(0) {
    ...
    if (do_paxos == true) {
        ...
        // paxos = new Paxos(conf->this_group, conf->this_node, paxos_connection,
                          // conf->this_node_partition, conf->num_partitions);
    } else
        batch_prop_limit = conf->all_nodes.size();
}

GenepiSequencer::~GenepiSequencer() {
...
void GenepiSequencer::FindParticipatingNodes(const TxnProto &txn, set<int> *nodes) {
    nodes->clear();
...
void GenepiSequencer::RunWriter() {
    Spin(1);
    ...
    while (!deconstructor_invoked_) {
        ...
        if (now > epoch_start_ + batch_count_ * epoch_duration_) {
            ...
            TxnProto *txn;
            int txn_base =
                batch_count_ * configuration_->all_nodes.size() + node_id;
...
void GenepiSequencer::DealWithMsg(MessageProto *message,
                            map<int, MessageProto> &batches) {
    SEQLOG(-1, " got single part msg for batch "
                   << batch_count_ << ", proposed batch is " << proposed_batch);
    ...
    for (int i = 0; i < message->data_size(); ++i) {
        ...
        // LOG(txn.txn_id(), " being processed, is multipart
        // "<<txn.multipartition());
        set<int> to_send;
        ...
        if (to_send.size() == 1 && *to_send.begin() == node_id) {
            ...
            for (set<int>::iterator it = to_send.begin(); it != to_send.end();
                 ++it) {
    ...
    if (num_pending[batch_number] == 0 && batch_number == proposed_batch + 1) {
        SEQLOG(-1, " proposing to global " << batch_number
                                           << ", proposed batch is "
                                           << proposed_batch);
        if (multi_part_txns.count(batch_number) != 0) {
            priority_queue<MessageProto *, vector<MessageProto *>, CompareMsg>
                msgs = multi_part_txns[batch_number];
            SEQLOG(-1, " Proposing to global "
                           << batch_number << ", msg size is " << msgs.size());
            while (msgs.size()) {
        ...
        SEQLOG(-1, " not ready to proceed " << batch_number
                                            << ", num pending is "
                                            << num_pending[batch_number]);
    ...
    if (involved_parts.size()) {
        ...
        pending_sent_skeen[batch_number] =
            MyFour<int64, int64, vector<int>, MessageProto *>(
                involved_parts.size(), 0, output, multi_part_msg);
    } else
        delete multi_part_msg;

    for (set<int>::iterator it = involved_parts.begin();
         it != involved_parts.end(); ++it) {
...
void GenepiSequencer::propose_global(
    ...
    while (true) {
        ...
        if (num_pending[next_batch] == 0 && pending_paxos_props.size() &&
            ...
            SEQLOG(-1, " Proposing to global " << next_batch
                                               << ", proposed batch is "
                                               << proposed_batch);
            if (multi_part_txns.count(next_batch) != 0) {
                priority_queue<MessageProto *, vector<MessageProto *>,
                               CompareMsg>
                    msgs = multi_part_txns[next_batch];
                SEQLOG(-1, " Proposing to global " << next_batch
                                                   << ", msg size is "
                                                   << msgs.size());
                while (msgs.size()) {
...
void GenepiSequencer::RunReader() {
    ...
    while (!deconstructor_invoked_) {
        ...
        while (skeen_connection_->GetMessage(&recv_msg)) {
            ...
            if (msg_type == MessageProto::SKEEN_REQ) {
                int64 to_propose_batch =
                    max(max_batch, proposed_batch + batch_pad);
                // Increase random_batch with 50% probability, to avoid the case
                // that messages keep being aggregated in this batch
                ...
                pending_received_skeen[recv_msg.msg_id()] =
                    new MessageProto(recv_msg);
                SEQLOG(-1, " replying skeen request: "
                               << recv_msg.msg_id() << ", proposing "
                               << to_propose_batch << ", now pending is "
                               << num_pending[to_propose_batch]);
                ...
                MyFour<int64, int64, vector<int>, MessageProto *> entry =
                    pending_sent_skeen[index];
                SEQLOG(-1, " Got skeen propose: "
                               << recv_msg.msg_id() << ", he proposed "
                               << recv_msg.propose_batch() << ", remaining is "
                               << entry.first);
                ...
                if (entry.first == 1) {
                    ...
                    int64 final_batch =
                        max(max_batch,
                            max(proposed_batch + batch_pad, entry.second));
                    // Increase random_batch with 50% probability, to avoid the
                    // case that messages keep being aggregated in this batch
                    ...
                    SEQLOG(-1, " Got skeen propose: "
                                   << recv_msg.msg_id() << ", pushed "
                                   << entry.fourth->msg_id() << " to"
                                   << final_batch << " and size is "
                                   << multi_part_txns[final_batch].size());
                    SEQLOG(-1, " For "
                                   << recv_msg.msg_id() << ", num pending is "
                                   << num_pending[final_batch]
                                   << ", proposed_batch is" << proposed_batch);

                    if (num_pending[final_batch] == 0 &&
                        proposed_batch + 1 == final_batch)
                        propose_global(proposed_batch, num_pending,
                                       pending_paxos_props, multi_part_txns);
                } else {
                    entry.first -= 1;
                ...
                int64 new_batch = recv_msg.batch_number(),
                      blocked_batch = pending_received_skeen[recv_msg.msg_id()]
                                          ->propose_batch();

                // Put it to the batch
                multi_part_txns[new_batch].push(
                    pending_received_skeen[recv_msg.msg_id()]);
                SEQLOG(-1, " got skeen final: "
                               << recv_msg.msg_id() << ", batch number is "
                               << new_batch << ", pushed "
                               << reinterpret_cast<int64>(
                                      pending_received_skeen[recv_msg.msg_id()])
                               << ", remain pending for batch " << blocked_batch
                               << " is " << num_pending[blocked_batch] - 1
                               << ", proposed batch is " << proposed_batch);
                num_pending[blocked_batch] -= 1;

                if (num_pending[blocked_batch] == 0 &&
                    blocked_batch == proposed_batch + 1) {
...
void GenepiSequencer::output(DeterministicScheduler *scheduler) {
