...
Paxos::Paxos(vector<Node *> &my_group, Node *myself_n,
    ...
    : group(my_group), myself(myself_n), num_partitions(num_p),
      partition_id(p_id), connection(paxos_connection) {
...
void Paxos::HandleClientProposal(MessageProto *message, int &batch_to_prop) {
    ...
    if (client_prop_map.count(batch_num) == 0) {
        ...
        // "<<reinterpret_cast<int64>(msgs.second[0]));  LOG(-1, "after storing is
        // "<<reinterpret_cast<int64>(msgs.second[1]));
    }
            ...
            if (message->type() == MessageProto::CLIENT_PROPOSAL) {
                ...
                if (message->batch_number() >= batch_to_accept) {
                    ...
                    while (leader_prop_map.count(batch_to_accept) != 0 &&
                        ...
                        // std::cout<<"Accepting batch "<<batch_to_accept<<",
                        // his batch is
                        // "<<leader_prop_map[batch_to_accept].second->batch_number()<<std::endl;
                        leader_prop_map[batch_to_accept]
                            .second->set_destination_channel("scheduler_");
                        leader_prop_map[batch_to_accept].second->set_type(
                            MessageProto::TXN_BATCH);
                        leader_prop_map[batch_to_accept]
                            .second->set_destination_node(myself->node_id);
...
void Paxos::SubmitBatch(MessageProto *batch_msg) {
    // Send batch to leader
    // std::cout<<"Sending msg of batch "<<batch_msg->batch_number()<<" to
    // paxos" <<std::endl;
    batch_msg->set_destination_node(leader->node_id);
    batch_msg->set_type(MessageProto::CLIENT_PROPOSAL);
    pthread_mutex_lock(&mutex_);
