...
class Paxos {
  public:
    ...
    Paxos(vector<Node *> &my_group, Node *myself_n,
          Connection *paxos_connection, int partition_id, int num_partitions);
    ...
    // thus inserting the batch into the global order. Once a quorum of
    // zookeeper nodes have agreed on an insertion, it will appear in the same
    // place in the global order to all readers.
