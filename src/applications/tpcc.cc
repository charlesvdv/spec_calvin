...
void TPCC::NewTxn(int64 txn_id, int txn_type, Configuration *config,
    ...
    if (mp) {
        // do {
            // remote_node = config->RandomPartition();
        // } while (config->this_group.size() > 1 &&
                 // remote_node == config->this_node_id);
        // remote_node = config->RandomDCNode();
        remote_node = distrib->GetPartitions(1)[0];

        do {
            remote_warehouse_id =
                rand() % (WAREHOUSES_PER_NODE * config->num_partitions);
    ...
    case NEW_ORDER: {
        ...
        warehouse_id = (rand() % WAREHOUSES_PER_NODE) * config->num_partitions +
        ...
        snprintf(district_key, sizeof(district_key), "w%dd%d", warehouse_id,
                 district_id);
        ...
        snprintf(customer_key, sizeof(customer_key), "w%dd%dc%d", warehouse_id,
                 district_id, customer_id);
        // 1st key in read set is customer
        txn->add_read_write_set(customer_key);

        // We set the length of the read and write set uniformly between 5 and
        // 15
        ...
        if (mp) {
            ...
            snprintf(remote_warehouse_key, sizeof(remote_warehouse_key), "%s",
                     warehouse_key);
        }
    ...
    case PAYMENT:
        ...
        tpcc_args->set_amount(
            rand() / (static_cast<double>(RAND_MAX + 1.0)) * 4999.0 + 1);
        ...
        district_id = rand() % DISTRICTS_PER_WAREHOUSE;
        snprintf(district_key, sizeof(district_key), "w%dd%dy", warehouse_id,
                 district_id);
        txn->add_read_write_set(district_key);

        // Add history key to write set
        char history_key[128];
        snprintf(history_key, sizeof(history_key), "w%dh%ld", warehouse_id,
                 txn->txn_id());
        ...
        if (WAREHOUSES_PER_NODE * config->all_nodes.size() == 1 || !mp) {
            ...
            snprintf(customer_key, sizeof(customer_key), "w%dd%dc%d",
                     remote_warehouse_id, remote_district_id,
                     remote_customer_id);
            txn->add_readers(remote_node);
            txn->add_writers(remote_node);
        }
    ...
    case DELIVERY: {
        ...
        for (int i = 0; i < DISTRICTS_PER_WAREHOUSE; i++) {
            snprintf(district_key, sizeof(district_key), "%sd%d", warehouse_key,
                     i);
            txn->add_read_write_set(district_key);
        }
        txn->add_readers(config->this_node_id);
...
int TPCC::NewOrderTransaction(StorageManager *storage) const {
    ...
    // LOG(txn->txn_id(), "Executing NEWORDER, is multipart?
    // "<<(txn->multipartition()));
    ...
    if (storage->ShouldExec()) {
        ...
        if (read_state == SUSPENDED)
            ...
            if (district.smallest_order_id() == -1) {
                ...
                // LOG(txn->txn_id(), "for "<<district_key<<", setting smallest
                // order id to be "<<order_number);
            }
    ...
    char order_key[128];
    snprintf(order_key, sizeof(order_key), "%so%d", district_key.c_str(),
             order_number);
    ...
    // LOG(txn->txn_id(), " before trying to write order "<<order_key<<",
    // "<<reinterpret_cast<int64>(order_value));
    storage->PutObject(order_key, order_value);

    char new_order_key[128];
    snprintf(new_order_key, sizeof(new_order_key), "%sno%d",
    ...
    for (int i = 0; i < order_line_count; i++) {
        ...
        if (storage->ShouldExec()) {
            ...
            if (read_state == SUSPENDED)
                ...
                // And we decrease the stock's supply appropriately and rewrite
                // to storage
        ...
        snprintf(order_line_key, sizeof(order_line_key), "%so%dol%d",
                 district_key.c_str(), order_number, i);
        ...
        // "<<order_line_key<<", "<<reinterpret_cast<int64>(order_line_val));
...
int TPCC::PaymentTransaction(StorageManager *storage) const {
    ...
    LOG(txn->txn_id(),
        "Executing PAYMENT, is multipart? " << (txn->multipartition()));
    ...
    // LOG(txn->txn_id(), " writing to warhouse "<<warehouse_key<<" of value
    // "<<*warehouse_val);
    ...
    if (storage->ShouldExec()) {
        ...
        //	customer_val += 1;
            ...
            if (customer.credit() == "BC") {
                ...
                snprintf(
                    new_information, sizeof(new_information), "%s%s%s%s%s%d%s",
...
int TPCC::OrderStatusTransaction(StorageManager *storage) const {
    TxnProto *txn = storage->get_txn();
    // LOG(txn->txn_id(), "Executing ORDERSTATUS, is multipart?
    // "<<txn->multipartition());
...
int TPCC::StockLevelTransaction(StorageManager *storage) const {
    TxnProto *txn = storage->get_txn();
    // LOG(txn->txn_id(), "Executing STOCKLEVEL, is multipart?
    // "<<txn->multipartition());  int threshold = tpcc_args.threshold();
    ...
    for (int i = latest_order_number;
         (i >= 0) && (i > latest_order_number - 20); i--) {
        char order_key[128];
        snprintf(order_key, sizeof(order_key), "%so%d", district_key.c_str(),
                 i);
...
int TPCC::DeliveryTransaction(StorageManager *storage) const {
    ...
    tpcc_args.ParseFromString(txn->arg());
    LOG(txn->txn_id(),
        "Executing DELIVERY, is multipart? " << txn->multipartition());
    ...
    for (int i = 0; i < DISTRICTS_PER_WAREHOUSE; i++) {
        ...
        LOG(txn->txn_id(), " before trying to read district "
                               ...
                               << reinterpret_cast<int64>(district_val));
        assert(district.ParseFromString(*district_val));
        // Only update the value of district after performing all orderline
        // updates
        if (district.smallest_order_id() == -1 ||
            district.smallest_order_id() >= district.next_order_id())
            continue;
        else {
            // LOG(txn->txn_id(), "for "<<district_key<<", setting smallest
            // order id to be "<<district.smallest_order_id());
            ...
            char new_order_key[128];
            snprintf(new_order_key, sizeof(new_order_key), "%sn%s",
                     district_key, order_key);
            ...
            for (int j = 0; j < order_line_count; j++) {
                ...
                // LOG(txn->txn_id(), " before trying to write orderline
                // "<<order_line_key<<",
                // "<<reinterpret_cast<int64>(order_line_val));
                assert(order_line.ParseFromString(*order_line_val));
            ...
            assert(customer.SerializeToString(customer_val));
            storage->PutObject(customer_key, customer_val);
            // LOG(txn->txn_id(), " before trying to write customer
            // "<<customer_key<<", value is
            // "<<reinterpret_cast<int64>(customer_val));

            // LOG(txn->txn_id(), " before trying to write district
            // "<<district_key<<", "<<reinterpret_cast<int64>(district_val));
...
void TPCC::InitializeStorage(Storage *storage, Configuration *conf) const {
    ...
    for (int i = 0; i < (int)(WAREHOUSES_PER_NODE * conf->all_nodes.size());
         i++) {
        ...
        for (int j = 0; j < DISTRICTS_PER_WAREHOUSE; j++) {
            ...
            snprintf(district_key_ytd, sizeof(district_key_ytd), "w%dd%dy", i,
                     j);
            ...
            // Finally, we pass it off to the storage manager to write to disk
            if (conf->LookupPartition(district_key) ==
                conf->this_node_partition) {
                storage->PutObject(district_key, district_value);
                storage->PutObject(district_key_ytd,
                                   new Value(*district_value));
            }
            ...
            for (int k = 0; k < CUSTOMERS_PER_DISTRICT; k++) {
                ...
                snprintf(customer_key, sizeof(customer_key), "w%dd%dc%d", i, j,
                         k);

                // Next we initialize the object and serialize it
                Value *customer_value = new Value();
                Customer *customer =
                    CreateCustomer(customer_key, district_key, warehouse_key);
                assert(customer->SerializeToString(customer_value));

                // Finally, we pass it off to the storage manager to write to
                // disk
                if (conf->LookupPartition(customer_key) ==
                    conf->this_node_partition)
...
Stock *TPCC::CreateStock(Key item_key, Key warehouse_key) const {
    ...
    // We initialize the various keys
    char stock_key[128];
    snprintf(stock_key, sizeof(stock_key), "%ss%s", warehouse_key.c_str(),
             item_key.c_str());
