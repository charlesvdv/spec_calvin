...
TEST(NewTxnTest) {
    ...
    for (int i = 0; i < tpcc_args->order_line_count(); i++)
        EXPECT_TRUE(tpcc_args->quantities(i) <= 10 &&
                    tpcc_args->quantities(i) > 0);
...
TEST(InitializeTest) {
    ...
    for (int i = 0; i < WAREHOUSES_PER_NODE; i++) {
        ...
        for (int j = 0; j < NUMBER_OF_ITEMS; j++) {
            ...
            snprintf(stock_key, sizeof(stock_key), "%ss%s", warehouse_key,
                     item_key);
...
TEST(NewOrderTest) {
    ...
    for (int i = 0; i < txn_args->order_line_count(); i++) {
        ...
        Key item_key =
            txn->read_write_set(i + 1).substr(item_idx, string::npos);
        Value item_value = *(tpcc->GetItem(item_key));
        Item *item = new Item();
        EXPECT_TRUE(item->ParseFromString(item_value));

        // Check the order line
        // TODO(Thad): Get order_line_ptr from Order protobuf and deserialize
        // from there
