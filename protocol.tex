...
switch, the switch initialization is aborded. The agreement is also used to agree on several values that will
...
\begin{description}
    \item[Transition phase (line \ref{alg:line:gen:transition}-\ref{alg:line:gen:transition:end})]
        ...
        with the genuine protocol for the switching partitions while the Calvin sequencing message will still be sent.
    ...
    \item[Waiting for required Calvin sequencing (line \ref{alg:line:gen:round})]
        ...
        needs to make sure that transaction no longer requires Calvin sequencing with the switching partition.
...
When one of the switching partition is a full genuine partition, the switching is really straightforward because
...
Each time a message is send from a partition to another, the communication must be a consensus to ensure
...
\begin{description}
    \item[Graphs locking] Before updating the round number of the graphs, the protocol switch needs to make
        ...
        every partition involved in the graph and, second, avoid switching with other partition inside the graph
        ...
        Each partition gives us more information about the graph.
    ...
    \item[Graphs round update propagation] Thanks to the round locking, the two switching
        ...
        be sent in every round update message.
    ...
    \item[LMEC Synchronization] Like in section \ref{sec:genuine-to-calvin-full-genuine}, before switching to the Calvin sequencer
        protocol, we need to switch to the transition protocol between the two switching partitions after upgrading the
        round number. The transition protocol will synchronize the various values of the \textit{CaMu} protocol.
