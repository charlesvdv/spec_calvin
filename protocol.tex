...
\begin{description}
    ...
    \item[Clock assignation phase (line \ref{alg:line:clock_assign_genuine} and \ref{alg:line:clock_assign_calvin}-\ref{alg:line:clock_assign_calvin:end})]
        ...
        % Those logical clock are ensured by \GE{} to be consistent across partitions and the
        % transaction are decided i
        % and are ensured to be
        % in total order between involved partitions.
    ...
    \item[Calvin sequencer value update (line \ref{alg:line:val_update}-\ref{alg:line:val_update:end})]
        ...
        % This timestamp should
        % be greater than any executed transactions' timestamp to avoid breaking total order.
        ...
        % This logical clock shouldn't have a smaller
        % value than any transaction's logical clock executed on any Calvin sequencer communicating partitions
        % during the last round to avoid breaking total order. As the MEC
        % is giving an agreement between partitions to bound transactions execution, the maximal LMEC value means that
        % every partition connected with the Calvin sequencer will be able to execute a transaction with a logical clock of
        % $calvin\_logical\_clock$ at least in the next round.
...
% TODO: move this another place in the thesis when I will talk about ordering.
...
A transaction can be dispatched to its involved partitions either only using one of the communication patterns, or using a combination of both, which we call a \textit{hybrid} dispatching. In the following text, we analyze whether the above cases are possible per dispatching type.
...
switch, the switch initialization is aborted \Li{Why can't this request be queued? In the current manner, is it possible that some switching may never be triggered (no progress)?}. The agreement is also used to agree on several values that will
be useful in the switch such as switching round, type of the partitions, \ldots Thoses values are stored in
a variable called $switch\_metadata$.
...
\begin{description}
    \item[Transition phase (line \ref{alg:line:gen:transition}-\ref{alg:line:gen:transition:end})]
        ...
        with the genuine protocol for the switching partitions while the Calvin sequencing message will still be sent \Li{why do we need to have both protocols running together? Isn't it enough to keep delivering message using Calvin until the switch round has reached?}.
    ...
    \item[Waiting for required Calvin sequencing (line \ref{alg:line:gen:round})]
        ...
        needs to make sure that transaction no longer requires Calvin sequencing with the switching partition \Li{when can we know this condition? And can't this round be known in advance in the previous phase?}.
...
\subsubsection{Pseudocode}
\Li{For the pseudo-code: when is switching round decided?}
...
\begin{description}
    ...
    \item[Calvin sequencer switch (line \ref{alg:line:calvin_1:switch}-\ref{alg:line:calvin_1:switch:end})]
        ...
        such as $calvin\_logical\_clock$ are updated before receiving transactions with the Calvin sequencer. \Li{why? Can't we use each partition's existing LMEC?}
...
\begin{description}
    \item[Graphs locking] Before updating the round number of the graphs, the protocol switch needs to make
        sure that no other switch occurs in the graph. This locking is required to, first, make a map of
        every partition involved in the graph and \Li{what do you mean 'make a map'? Do you mean just keeping track of the partitions?}, second, avoid switching with other partition inside the graph
        to avoid unknown partition in the mapping. If there is already a switching occurring in the graph,
        the switch needs to be aborted and every partition locked should also be aborted.

        The graph locking can be implemented in different ways. We choose an implementation where the switching
        partitions are doing the heavy lifting which is easier to manage and reason about. The locking/mapping
        protocol works by asking to every partitions its neighbours which are communicating with the Calvin sequencer.
        Each partition gives us more information about the graph \Li{Try to be more specific. Who sends lock message? When is locking messages sent?}.
    ...
    \item[Switching round agreement] A bit like in section \ref{sec:calvin-to-genuine}, the two switching partitions are going
        to agree on values \Li{The following two variables are not listed in the previous section. If they are also needed by the previous section, you should list them above already}:

        \begin{description}
            \item[Switching round] is the round at which the two graphs will switch.
            \item[Final round] is the final round value after the switching
        \end{description}

        Two values are required because the switching round can be different between the two switching partitions if
        there is two distinct graphs. A different value may be required because the two graphs could have a distant round number.
        If the value are distant and we have a common switching round, one partition will need to wait before the other partition
        is catching up its round lag \Li{Not clear. When will the waiting start? After switching been triggered, or before that?}.
    ...
    \item[LMEC Synchronization] Like in section \ref{sec:genuine-to-calvin-full-genuine}, before switching to the Calvin sequencer
        protocol, we need to switch to the transition protocol between the two switching partitions \Li{Just the two partitions, or also all connected partitions?} after upgrading the
        round number. The transition protocol will synchronize the various values of the \textit{CaMu} protocol. \Li{Overall, can't LMECs be synchronized together during switching round agreement phase?}
...
involved in the graph \Li{replace launch graph locking with a few lines of brief brief description would be more clear}.
