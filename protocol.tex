...
\section{General idea} \label{sec:challenges}
   ...
   To mitigate this problem, we can associate transactions dispatched by \PE{} a
   timestamp, just like \GE{}. Partitions now have a common way to order transactions.
   As timestamps must stay consistent across partitions, a transaction must have the same timestamp
   in every partition to ensure total order. Thus, transactions are always first dispatched with \GE{}
   to assign a timestamp to partitions using \GE{} and then dispatched with \PE{} to partitions
   using the \PE{} protocol. As transactions are first assigned a timestamp with \GE{}, the
   timestamp will always stay consistent.

   In case where transactions are only dispatched with \PE{}, transactions are
   assigned a timestamp which must be greater than any timestamps executed on
   \PE{} connected partitions. Otherwise, a \PE{} dispatched transaction could
   break the total order by having a smaller timestamp than an already executed
   transaction. \\

   Since transactions now have a consistent timestamp across partitions, partitions must make sure to
   execute transactions in total order by making sure no transaction with a smaller timestamp compared
   to any executed transactions' timestamp will ever be received.
