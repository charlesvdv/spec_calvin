...
\section{General idea} \label{sec:challenges}
   ...
   \GE{} assigns a timestamp to a transaction by using an agreement between the involved partitions
   which means \GE{} can't deterministically know the future timestamp of a transaction. On the other hand,
   \PE{} imposes a timestamp decided by the transaction receiver which means the transaction
   can be assigned any timestamp. In the case where a transaction requires \GE{} and \PE{}
   for the dispatching and ordering, the transaction will be first assigned a timestamp
   with \GE{}. When the transaction has a final timestamp value assigned by \GE{}, \PE{}
   can impose the final timestamp to partitions using the \PE{} protocol.

   % As \GE{} works by assigning a timestamp to a partition in a non-deterministic way (i.e.
   % the \GE{} protocol doesn't know before assigning the )
   % and \PE{} imposes a deterministic timestamp decided by the transaction receiver,
   % \GE{} will be used to assign the non-deterministic timestamp to the transaction and then
   % \PE{} will send the transaction to the remaining partition.
   ...
   indefinitely a message from the other partition and thus, block the ordering.
   ...
   On the other hand, when both partitions are using \PE{} and the two partitions are not in the
